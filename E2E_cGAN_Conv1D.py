# Original source code can be accessed at https://github.com/haoyye/End2End_GAN/blob/master/End2EndConvAWGN.py
...
def decoding(x):
    with tf.variable_scope("decoding", reuse=tf.AUTO_REUSE):
        ...
        conv3 += conv3_ori
        conv3 = tf.nn.relu(conv3)
        conv4 = tf.layers.conv1d(inputs=conv3, filters=64, kernel_size=3, padding='same')
...
def generate_batch_data(batch_size):
    ...
    batch_s = data[start_idx:start_idx + batch_size]
...
# Transmitter
s = tf.placeholder(tf.float32, shape=[None, block_length, 1])
cond = encoding(s)
Z = tf.placeholder(tf.float32, shape=[None, block_length, Z_dim_c])
Noise_std = tf.placeholder(tf.float32, shape=[])

# Receiver
R_sample = gaussian_noise_layer(cond, Noise_std) #real sample AWGN
G_sample = generator_conditional(Z, cond)
...
R_sample_uniform = gaussian_noise_layer(encodings_uniform_generated, Noise_std) # AWGN channel
...
loss_receiver_G = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
    logits=G_decodings_logit, labels=s))
...
loss_receiver_R = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
    logits=R_decodings_logit, labels=s))
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
Rx_solver = optimizer.minimize(loss_receiver_R, var_list=Rx_vars)


# Metrics
accuracy_R = tf.reduce_mean(tf.cast((tf.abs(R_decodings_prob - s) < 0.5), tf.float32))
WER_R = 1 - tf.reduce_mean(tf.cast(tf.reduce_all(tf.abs(R_decodings_prob - s) < 0.5, 1), tf.float32))
WER_R_BLER = 1 - tf.reduce_mean(tf.cast(tf.reduce_all(tf.reshape(tf.abs(R_decodings_prob - s) < 0.5, [-1, block_length]), axis=-1), tf.float32))
accuracy_G = tf.reduce_mean(tf.cast((tf.abs(G_decodings_prob - s) < 0.5), tf.float32))
WER_G = 1 - tf.reduce_mean(tf.cast(tf.reduce_all(tf.abs(G_decodings_prob - s) < 0.5, 1), tf.float32))

init = tf.global_variables_initializer()
number_steps_transmitter = 100
number_steps_receiver = 100
number_steps_channel = 40
display_step = 100
batch_size = 320
...
with tf.Session(config=config) as sess:
    ...
    for iteration in range(number_iterations):
        ...
        for step in range(number_steps_channel):
            ...
            batch_s = generate_batch_data(int(batch_size / 2))
            encoded_data = sess.run([cond], feed_dict={s: batch_s})
        ...
        for step in range(number_steps_transmitter):
            ...
            batch_s = generate_batch_data(batch_size)
            sess.run(Tx_solver, feed_dict={s: batch_s,
                                           Z: sample_Z([batch_size, block_length, Z_dim_c]),
        ...
        for step in range(number_steps_receiver):
            ...
            batch_s = generate_batch_data(batch_size)
            sess.run(Rx_solver, feed_dict={s: batch_s, Noise_std: (np.sqrt(1 / (2 * R * EbNo_train)))})


        '''  ----- Testing ----  '''
        loss, acc = sess.run([loss_receiver_R, WER_R_BLER],
                             feed_dict={s: batch_s, Noise_std: np.sqrt(1 / (2 * R * EbNo_train))})
        print("Real Channel Evaluation:", "Step " + str(step) + ", Minibatch Loss= " + \
              "{:.4f}".format(loss) + ", Training Accuracy (WER_R_BLER) = " + \
              "{:.3f}".format(acc))

        loss, acc = sess.run([loss_receiver_G, accuracy_G],
                             feed_dict={s: batch_s, Z: sample_Z([batch_size, block_length, Z_dim_c]),
                                        Noise_std: np.sqrt(1 / (2 * R * EbNo_train))
                                        })
        ...
        EbNodB_range = np.arange(-10, 15, 1)
        bler = np.ones(len(EbNodB_range))
        for n in range(0, len(EbNodB_range)):
            EbNo = 10.0 ** (EbNodB_range[n] / 10.0)
            bler[n] = sess.run(WER_R_BLER,
                                  feed_dict={s: test_data, Noise_std: (np.sqrt(1 / (2 * R * EbNo)))})

        print('SNR:\n{}\nBLER:\n{}'.format(EbNodB_range, bler))
